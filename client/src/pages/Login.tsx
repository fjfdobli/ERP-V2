...
const Login = () => {
  ...
  const [directLoginError] = useState('');
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    console.log('Form submitting with email:', email);
    
    // Ensure the values are properly captured
    const loginValues: LoginFormValues = { 
      email: email.trim().toLowerCase(), 
      password: password // Keep password exactly as entered
    };
  ...
  <defs>
    <!-- Base pattern for background texture -->
  ...
  <!-- Modern Offset Printing Press (Bottom Left) -->
  ...
  <!-- Digital Printing Machine (Top Right) -->
  ...
  <!-- Printing Plates and Supplies (Middle Center) -->
  ...
  <!-- Large Format Printer (Bottom Right) -->
  ...
  <!-- Ink Bottles and Supplies (Middle Left) -->
  ...
  <!-- Desktop Publishing Workstation (Middle Right) -->
  ...
  <!-- Conveyor with printed materials (Bottom Center) -->
  ...
  <!-- Very subtle color overlay for visual interest -->
  <rect width="1200" height="800" fill="url(#cmykGradient)" opacity="0.02"/>
  
  <!-- Scattered papers (various locations) -->
  <g opacity="0.5">
    <!-- Paper 1 -->
        ...
        >
          <Grid container>
            ...
            <Grid item xs={12} sx={{ p: 4 }}>
              ...
              {directLoginError && (
                <Alert severity="error" sx={{ mb: 3 }}>
                  Direct login error: {directLoginError}
                </Alert>
              )}
